...
IF @@TRANCOUNT>0 BEGIN COMMIT TRAN END
    ...
    Exec spPagosDocumentos  @Op='I',@IDen_Documentos=@Iden_Documento_Select,@FormaPago='EFE',@Valor={ValorTotalPedido},@Cambio=0,@Comision=0,@Retencion=0,@IvaInformado={TotalIvaDef},@ReteIVA=0,
@Vencimiento='{Fecha}',@Cliente='99        ',@Banco='',@Plaza='',@DocumentoPago='',@EsUnAnticipo=0
    
    """.format(IdDocumento =IdDocumento,ValorTotalPedido =float( Pedido['Info']['ValorPedido'] ),Fecha=Fecha,TotalIvaDef=TotalIvaDef)
    QueryConeptiosIcos = """
...
@Moneda = '{MedioPago}', @TasaCambio = 1, @Cliente = '99        ', @Vendedor = '82 ', @UsuariodeServicio = '99-{IdentificacionUsuario}', @NombreUsuariodeServicio = 'Turista   ',
...
{QueryConsecutivosEx}
    ...
    """.format(MedioPago=Pedido['Info']["CodigoPago"], QueryConsecutivosEx=QueryConsecutivos,IdentificacionUsuario=IdentificacionUsuario,TurnoPedido=Turno,FechaPedido=Fecha,ItesmPedidoQuery=QueryItems,MedioPagoQuery=QueryMedioPago,ConceptosQuery=QueryConeptiosIcos,IdentificacionQuery=QueryIdentificacionUsuario)
    #print("PEDIDO::","\n",LargeConsulta)
...
r = requests.get( UrlApi+'/api/mobile/GetPedidosDisponibles')
